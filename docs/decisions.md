...
## 2026-07-31 — Participant Recall Is Per-Participant, Comparable Only Per-Run
**Status:** Active
**Domain:** Technical / Mechanism
**Context:** Panel turns are stateless provider calls — `complete(system, user)`, no accumulated message list. The public thread is re-read from the platform and re-rendered in full every turn, so a participant always sees the whole conversation verbatim. What was unspecified is narrower: whether a participant sees its own prior *private* `reasoning` when its turn comes round again. Under no recall, a participant can see that it argued something three turns ago but not why — less continuity than any human in a meeting has. Under recall, it carries its own thread of thought, and also its own early errors.
**Options considered:** (a) No recall, always — simplest, keeps the reasoning trail clean as a contemporaneous record to check the retrospective survey against; (b) Recall always; (c) Per-participant setting, with some participants in a run carrying recall and others not; (d) Per-participant mechanism, but uniform per run for any actual comparison.
**Decision:** Option (d). `Bot.recall` is `none` (default) or `own`, set per participant in `conditions.py`. `Condition.with_recall()` produces a uniform copy leaving names, slugs and tiers untouched. `run.py --recall {none,own,mixed}`: `none` and `own` force every participant uniformly, `mixed` honours the roster. Recall never crosses participants — nobody sees anyone else's reasoning. `metadata.json` records the resolved per-participant modes, not just the flag. A pilot mixed assignment ships in condition A only.
**Reasoning:** Neither (a) nor (b) is obviously the right model of a human deliberator, so measuring both beats guessing. Option (c) as the primary design is confounded: with three participants, recall is entangled with model identity — Claude-with-notes versus GPT-without is two variables moving at once and nothing in the transcript separates them — and the participants are not independent samples, since they share one conversation and a participant with better continuity changes what the others respond to. Making the mechanism per-participant costs nothing extra and is strictly more general, since a uniform run is just every participant on the same setting; making the *comparison* per-run is what keeps a result interpretable. `mixed` is kept because a confounded look at whether recall visibly changes anything is worth having before spending two runs finding out. Recall is delivered as a text block in the user prompt rather than a chat history — the provider interface has nowhere to put one — which makes it behave more like a person re-reading their own notes than like a model that remembers.
**Implications:** The clean comparison is two runs with the same `--thread-prompt`, one `none` and one `own`. Mixed runs are pilots and must be read as such; `design.md` says so and so does the flag name. The pilot assignment lives in condition A because its display names make the name → provider mapping unambiguous; condition B stays uniform `none` until Sprint 2 fixes which provider backs which neutral name, since assigning recall before that would be assigning it blind. Recall under `own` partly turns the `reasoning` field into working memory, which weakens its secondary use as a contemporaneous record to check the Stage 1 survey against — another reason the `none` arm has to exist.
**Superseded in part by:** 2026-07-31 — Crossed Rosters and Seed-Time vs Run-Time Factors, which shows the confound came from one-bot-per-provider rather than from mixing, and adds a crossed roster that dissolves it.

---

## 2026-07-31 — Crossed Rosters, and Seed-Time vs Run-Time Factors
**Status:** Active
**Domain:** Technical / Mechanism
**Context:** The previous entry claimed that any run mixing recall modes was confounded, and therefore that recall could only be compared between uniform runs. That was too strong. It also left no way to ask a related question the design should support: does knowing *which* provider backs a co-participant change how a model argues?
**Options considered:** (a) Keep the earlier position — mixing is always confounded, compare only between runs; (b) Add participants per provider so recall can be varied within a provider, and check the balance rather than assert it; (c) Add naming variants as a run-time prompt flag; (d) Add naming variants as separate seeded accounts grouped into rosters.
**Decision:** (b) and (d). `Bot` gains `provider`. A new `Roster` groups a disjoint set of accounts inside one condition's community, carrying a `naming` style and its own bots. `Roster.is_crossed()` is true only when every provider fields a participant in each recall mode. Condition A ships three rosters — `named` (1/provider), `crossed` (2/provider, recall balanced within each), `anonymous` (1/provider, provider identity hidden in the display names). `seed.py` gains `--roster` and `--list`; `run.py` will gain `--roster` and warn when `--recall mixed` is used on an uncrossed roster.
**Reasoning:** The confound was real but came from roster *shape*, not from mixing. With one participant per provider, assigning recall to some participants necessarily assigns it to some providers — the factors cannot be separated at any assignment, which `test_one_bot_per_provider_cannot_be_crossed` now pins. Two per provider makes it an ordinary 2×3 factorial where recall and provider vary independently, and comparing recall arms within a provider is exactly the wanted comparison. Making that a computed property rather than a comment means a roster edit that breaks the balance fails a test instead of quietly producing an uninterpretable run. Option (c) is impossible, not merely worse: `display_name` is a column on a platform account and `render_thread_state` emits the platform's own name, so a participant called `Claude Panel` announces its provider every turn and no prompt flag can un-say it. That forces the seed-time/run-time split — naming and roster composition are account facts and must be seeded; recall, co-participant framing, and provider disclosure in prompt text are assembly and switch per run for free. Rosters share a community rather than needing one each because `render_thread_state` renders a thread, not a member list, so participants in different threads never see each other; only the community *slug* forces separate communities, which is why A and B differ and A's three rosters do not.
**Implications:** Seeding all rosters is 23 bot accounts rather than 12, on a real platform with a public audit log — hence `--roster` for seeding only what a planned run needs, and `--list` for reading the design without a network call. `bot_slug` must now be unique condition-wide, since env var names derive from it alone; a test enforces it. Sprint 2 must record condition, roster key, naming style and resolved per-participant recall in `metadata.json`, and its leak check on a blind or anonymous run must grep the assembled prompt for every provider name, not just condition A's display names. A crossed roster makes a mixed run interpretable, not powered — one run is still an observation.
**Revisit if:** Provider count changes, or local models arrive via `OpenAICompatibleProvider`, at which point `PROVIDER_LOCAL` participants need their own crossing decision.
